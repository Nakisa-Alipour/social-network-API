...
const thoughtController = {
    ...
    getThoughtById({ params }, res) {Thought.findOne(
    ...
    updateThought({ params, body }, res) {Thought.findOneAndUpdate(
        { _id: params.thoughtId }, 
        body, 
    ...
    deleteThought({ params }, res) {Thought.findOneAndDelete(
        { _id: params.thoughtId })
        .then((dbThoughtData) => {
            // If no thought found, send 404 status and error message
            if (!dbThoughtData) {
    ...
    createReaction({ params, body }, res) {Thought.findOneAndUpdate(
        { _id: params.thoughtId },
        { $push: { reactions: body } },
        { new: true, runValidators: true })
        .then((dbThoughtData) => {
    ...
    deleteReaction({ params }, res) {Thought.findOneAndUpdate(
      { _id: params.thoughtId },
