...
const userController = {
    ...
    getUserById({ params }, res) {User.findOne({ _id: params.userId })
        .populate({
            path: 'thoughts',
            select: '-__v' // Exclude the "__v" field from the populated thoughts
        })
    ...
    // Update a user by ID
    updateUser({ params, body }, res) {User.findOneAndUpdate({ _id: params.userId }, body, {new: true, runValidators: true})
        .then((dbUserData) => {
        // If no user found, send 404 status and error message
        if (!dbUserData) {
            res.status(404).json({ message: 'No user found with this id!' });
    ...
    // Delete a user by ID
    deleteUser({ params }, res) {User.findOneAndDelete({ _id: params.userId })
    .then((dbUserData) => {
        // If no user found, send 404 status and error message
        if (!dbUserData) {
            res.status(404).json({ message: 'No user found with this id!' });
    ...
    addFriend({ params }, res) {User.findOneAndUpdate({ _id: params.userId },{ $push: { friends: params.friendId } },{ new: true })
    .then((dbUserData) => {
        // If no user found, send 404 status and error message
        if (!dbUserData) {
            res.status(404).json({ message: 'No user found with this id!' });
    ...
    deleteFriend({ params }, res) {User.findOneAndUpdate({ _id: params.userId },{ $pull: { friends: params.friendId } }, { new: true })
    .then((dbUserData) => {
        // If no user found, send 404 status and error message
        if (!dbUserData) {
           res.status(404).json({ message: 'No user found with this id!' });
